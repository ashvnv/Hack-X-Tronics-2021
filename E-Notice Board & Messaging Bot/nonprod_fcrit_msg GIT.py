...
def IFTTT_EVENT(val1,val2,val3):
    myobj = {"value1":val1,"value2":val2,"value3":val3}
    url = '<>' #Webhooks url removed
            ...
            if v == 'NA':
                ...
                for i,j in json_dictionary.items():
                    if re.search('^#.+',j):
                        ...
                        bot.sendMessage(v,j)
                        if IFTTT_LOGGED == False:
                            IFTTT_EVENT('NA',rx,j)
                        print('message sent')
                IFTTT_LOGGED = True # only log once
